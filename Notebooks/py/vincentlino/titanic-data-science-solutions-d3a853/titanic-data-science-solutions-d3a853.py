...
# fill age na with median group by title
pieces = [train_df.drop("Survived", axis=1), test_df]
all_sample = pd.concat(pieces)

title_age_df = all_sample[['Age', 'Title']].dropna(subset=['Age']).groupby(['Title'], as_index=False).median()

train_df = train_df.combine_first(train_df.set_index('Title').Age.fillna(title_age_df.set_index('Title').Age).reset_index())
train_df['Age'] = train_df['Age'].astype(int)

test_df = test_df.combine_first(test_df.set_index('Title').Age.fillna(title_age_df.set_index('Title').Age).reset_index())
...
# 
# **Warning:** discrete categorical variables into ordinal numerical features
...
# [LR模型简介](http://tech.meituan.com/intro_to_logistic_regression.html)
# 
# Quiz:
# 1. 如何判断损失函数是凸函数？ 
# 2. 数据分布不均衡如何解决？ 
...
print('the intercept of regression equation is:',logreg.intercept_[0])
...
# Gridsearch and tuning an randomforrest algorithm
...
#  
# [User Guide](http://xgboost.readthedocs.io/en/latest/model.html)
...
acc_xgb = round(gbm.score(X_train, Y_train) * 100, 2)
acc_xgb
