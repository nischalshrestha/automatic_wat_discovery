...
print(titanic_df[numeric_features].head())
...
for i, max_depth in enumerate(max_depth_arr):
    test = DecisionTreeClassifier(random_state=42, max_depth=max_depth, criterion='entropy')
    test.fit(train_X, train_y)
    ## calcula accuracy usando o max_depth em questao
    accuracy_arr[i] = test.score(train_X, train_y) # coloque aqui o seu calculo de accuracy
    print(test.score(train_X, train_y),max_depth)
...
test = DecisionTreeClassifier(random_state=42, max_depth=optimal_max_depth)
test.fit(train_X, train_y)
