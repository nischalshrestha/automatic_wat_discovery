...
# # Titanic Competition: A Complete Explanation
# 
# This notebook was originially forked from "[Titanic Data Science Solutions](https://www.kaggle.com/startupsci/titanic-data-science-solutions)" and then modified for clarification.
# 
...
# 
# **Correlation** One can approach the problem based on available features (also known as input variables or parameters) within the training dataset. Which features within the dataset contribute significantly to our solution goal? Statistically speaking is there a correlation among a feature and solution goal? As the feature values change does the solution state change as well, and visa-versa? This can be tested both for numerical and categorical features in the given dataset. We may also want to determine correlation among features other than survival for subsequent goals and workflow stages. Correlating certain features may help in creating, completing, or correcting features.
# 
# **Using Dummy variables** For the modeling stage, depending on the choice of model algorithm, one may require all categorical features to be converted to numerical equivalent values. For example, converting text values to numeric values so that it can be input into our training model.
# 
# **Complete Missing Data ** Data preparation also require us to replace any missing values within a feature with a value that makes sense.
# 
# **Correcting** We may also analyze the given training dataset for errors or possibly innacurate values within features and try to correct these values or exclude the samples containing errors. One way to do this is to detect any outliers among our parameters. We may also completely discard a feature if it is not contribting to the analysis or may significantly skew the results.
# 
# **Feature Engineering** Can we create new features based on an existing feature or a set of features, such that the new feature follows the correlation and completion goals.
# 
# **Plotting** This is selecting the proper visualization charts depending on nature of the data and the solution goals.
...
# "combine" is used to perform one operation on both datasets simultaneouly
# throughout this notebook, you'll see the "for" function when we are performing this operation


# ## Analyze by describing data
# 
# The Pandas library from Python also helps describe the datasets answering following questions early in our project.
# 
...
# 
# - Categorical Nominal: Survived, Sex, and Embarked. 
...
# - "Survived" is a categorical feature and the output variable with a 0 or 1 value.
...
# **Correlating.**
# 
# We want to know how well does each feature correlate with Survival. We want to do this early in our project and compare these quick correlations with correlations later in the project after we make our model.
# 
# **Completing.**
# 
...
# 
# **Correcting and Dropping Variables.**
# 
# 1. Ticket feature may be dropped from our analysis as it contains high ratio of duplicates (22%) and there may not be a correlation between Ticket and survival.
# 2. Cabin feature may be dropped as it is highly incomplete or contains many null values both in training and test dataset.
# 3. PassengerId may be dropped from training dataset as it does not contribute to survival.
# 4. Name feature is relatively non-standard, may not contribute directly to survival, so it can be dropped.
# 
# **Creating.**
# 
...
# 
# To confirm some of our observations and assumptions, we quickly analyzed our feature correlations by pivoting features against each other with the .groupby method. We can only do so at this stage for features which do not have any empty values. It also makes sense doing so only for features which are categorical (Sex), ordinal (Pclass) or discrete (SibSp, Parch) type.
# 
...
# **Observations.**
# 
# - Most Infants (Age <=4) had survived. 
...
# 
# Based on our assumptions and decisions we want to drop the Cabin (correcting #2) and Ticket (correcting #1) features.  These 2 features have little to no influence on whether a passenger survived or not.
# 
...
# 
# We want to analyze if the Name feature can be engineered to extract titles and test correlation between titles and survival, before dropping Name and PassengerId features.
# 
# In the following code we extract Title feature using regular expressions. The RegEx pattern `(\w+\.)` matches the first word which ends with a dot character within the Name feature. The `expand=False` flag returns a DataFrame.
# 
# **Observations.**
# 
...
# Here we replace uncommon Titles with the value "Rare"
...
# 
# 1. A simple way is to generate numbers from the mean of this column.
# 
# 2. A more accurate way of guessing missing values is to use other correlated features. In our case we note correlations among Age, Gender, and Passgener Class. Guess Age values using median values for Age across sets of Pclass and Gender feature combinations. So, median Age for Pclass=1 and Gender=0, Pclass=1 and Gender=1, and so on...
# 
# 3. Combine methods 1 and 2. So instead of guessing age values based on median, use numbers based on means from sets of Pclass and Gender.
# 
# Method 1 and 3 will introduce random noise into our models. The results from multiple executions might vary. We  prefer method 2.
