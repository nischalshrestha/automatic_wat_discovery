...
# 
# As we saw that maximum passengers boarded from Port S, we replace NaN with[](http://) S.
...
print('The accuracy of the DecisionT Tree is',metrics.accuracy_score(prediction4,test_Y))
...
plt.title('Average CV Mean Accracy')
fig=plt.gcf()
...
print('The cross validated score bagged KNN is:',result.mean())
...
result=cross_val_score(model,X,Y,cv=10,scoring='accuracy')
print('The cross validated score for bagged Decison Tree is:',result.mean())
