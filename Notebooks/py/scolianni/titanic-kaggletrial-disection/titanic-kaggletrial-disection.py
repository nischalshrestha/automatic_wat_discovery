...
def new_fam_size(df):
    df['Fam_Size'] = np.where((df['SibSp']+df['Parch']) == 0 , 'Solo', np.where((df['SibSp']+df['Parch']) <= 3,'Nuclear', 'Big'))
    del df['SibSp']
    del df['Parch']
    return df
...
def new_cabin_num(df):
    ...
    df['Cabin_num'] = pd.qcut(df['Cabin_num1'],3)
    df = pd.concat((df, pd.get_dummies(df['Cabin_num'], prefix = 'Cabin_num')), axis = 1)
    del df['Cabin_num']
    del df['Cabin_num1']
    return df
...
train = new_names(train)
test = new_names(test)

#train, test = age_impute(train, test)
train = new_age_impute(train)
test = new_age_impute(test)

#train, test = fam_size(train, test)
train = new_fam_size(train)
test = new_fam_size(test)

#train, test = ticket_grouped(train, test)
train = new_ticket_grouped(train)
test = new_ticket_grouped(test)

#train, test = cabin_num(train, test)
train = new_cabin_num(train)
test = new_cabin_num(test)

#train, test = cabin(train, test)
train = new_cabin(train)
test = new_cabin(test)

#train, test = embarked_impute(train, test)
train = new_embarked_impute(train)
test = new_embarked_impute(test)
