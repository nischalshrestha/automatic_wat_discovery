...
# **Pclass:** It has a negative correlation of -0.33 which means that increase in Pclass leads to decrease in Survived.
...
# People with 0-2 SibSp(Sibling/Spouse) have a higher chance of survival. The combined Siblings and Spouse data into a single feature brought the scary idea of two spouses into my mind. Yikes.
...
# Explanantion for the next code block used to get the Titles
...
# Fixing the missing data - First, we will focus on removing the null fields. Lets see all the features with nulls again.
...
# **Fare**
# 
# We saw earlier that the validation dataset has a null Fare. No null fares in the train dataset.
...
# Next up, we will encode all the categorical features. One of things to lookout for is the Dummy Trap. We have the option for using "drop_first = True" with get_dummies.
...
# 
# Major tasks pending-
# 1. Check impact of binning on Age and Fare.
# 2. Use Ticket.
# 3. Try other models and model tuning.
# 4. Create more visualizations.
# 5. Use K-Fold Cross Validation.
# 6. Try using the entire train data for training. This should theoretically improve the accuracy further as more data will be used to train the final model.
