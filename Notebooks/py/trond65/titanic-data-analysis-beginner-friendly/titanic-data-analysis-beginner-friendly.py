...
# Forked from Jicksy John's tutorial. Any subsequent errors are mine alone.
...
cabin = train[['Cabin', 'PassengerId']]
cabin_idx = cabin.set_index('Cabin')
#cabin
for index, row in train.iterrows():
    if (row['Cabin'] == 'B28'):
        print(cabin_idx.loc[row['Cabin']])
...
train["Age"] = train["Age"].fillna(random.randint(0,80))
...
train["Embarked"] = train["Embarked"].fillna(random.choice('SSSCQ'))
...
test["Age"] = test["Age"].fillna(random.randint(0,80))
