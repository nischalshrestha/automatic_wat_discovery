...
# easier to read than typical df.columns 
...
# Very good method of 'df.map(dict_mapping)'
...
#useful method pd.cut(columns,bins)
...
# ![](http://)We can remove the AgeBand feature.
...
#very simplified 1 line of code to fill in single value, dont' forget inplace=True
test_df['Fare'].fillna(test_df['Fare'].dropna().median(), inplace=True)
test_df.head()


# ![](http://)We will create FareBand.
...
# # Area of improvement  
# 1. Use cross validation to find top 3 baseline model
# 2.  Use hyperparameter tuning by gridsearch CV and select the best one
...
submission.to_csv('../working/basic_submission.csv', index=False)
