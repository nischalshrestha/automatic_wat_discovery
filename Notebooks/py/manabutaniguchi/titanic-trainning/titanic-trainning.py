...
#grid=sns.FacesGrid(train_df,col='Pclass',hue='Survived')
grid=sns.FacetGrid(train_df, col='Survived', row='Pclass',size=2.2,aspect=1.6)
grid.map(plt.hist,'Age', alpha=.5, bins=20)
grid.add_legend();
...
#grid=sns.FacetGrid(train_df,col='Embarked',hue='Survived',paleete{0:'K':1:'W'})
grid = sns.FacetGrid(train_df,row='Embarked',col='Survived',size=2.2,aspect=1.6)
grid.map(sns.barplot,'Sex','Fare',alpha=.5,ci=None)
grid.add_legend()
...
for dataset in combine:
    for i in range(0,2):
        for j in range(0,3):
            ...
            #age_men = guess_df.mean
            #age_std = guess_df.std
            #age_guess = rnd.uniform(age_mean - age_atd, age_mean + age_std)
...
models = pd.DataFrame({
    'Model': ['Support Vector Machines', 'KNN', 'Logistic Regression', 'Random Forest', 'Naive Bayes', 'Perceptron', 'Strochastic Grandient Decent', 'Linear SVC', 'Decision Tree'],
