...
train_df.describe(percentiles=[0.01,.1, .2, .3, .4, .5, .6, .7, .8, .9, .99])
...
train_o_df = train_df.drop(['Ticket', 'Cabin'], axis=1)
test_o_df = test_df.drop(['Ticket', 'Cabin'], axis=1)
combine_o = [train_o_df, test_o_df]

print("After", train_o_df.shape, test_o_df.shape, combine_o[0].shape, combine_o[1].shape)
...
for dataset in combine_o:
    dataset['Title'] = dataset.Name.str.extract(' ([A-Za-z]+)\.', expand=False)
    #print(dataset['Title'])

pd.crosstab(train_o_df['Title'], train_o_df['Sex'])
...
train_o_df[['Title', 'Survived']].groupby(['Title'], as_index=False).mean()
