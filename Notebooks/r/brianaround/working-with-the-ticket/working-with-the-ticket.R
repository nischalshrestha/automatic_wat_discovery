...
extractFeatures <- function(data) {
  ...
  #fea$CabinCount <- as.factor(sapply(strsplit(as.character(data$Cabin)," "), length))

  fea$TicketPrefix <- sub("\\s.*","",as.character(data$Ticket))

  fea$TicketPrefix <- gsub("[.]","",as.character(fea$TicketPrefix))

  fea$TicketPrefix[sapply(strsplit(as.character(train$Ticket), " "), length) == 1] <- ""

  #fea$HasTicketPrefix <- fea$TicketPrefix != ""
